...
public class User {
	...
	private Connection connect() {
		...
		try {
			...
			con = DriverManager.getConnection("jdbc:mysql://127.0.0.1:3307/userdb", "root", "");
		} catch (Exception e) {
			e.printStackTrace();
		}
		return con;
		...
		try {
			...
			while (rs.next()) {
				...
				output += "<td><input name='btnUpdate' type='button' value='Update' class='btn btn-secondary'></td>"
						...
						+ "<input name='itemID' type='hidden' value='<itemData><itemID>" + ID + "</itemID></itemData>'>"
						+ "</form></td></tr>";
	...
	public String deleteUser(String id) {
		...
		try {
			...
			// create a prepared statement
			String query = "";
			PreparedStatement preparedStmt = con.prepareStatement("delete from user where Id=?");
